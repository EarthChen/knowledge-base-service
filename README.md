...
- **混合搜索** — 关键词（FalkorDB）+ 语义 + 可选子块搜索，**RRF 融合**、可选**交叉编码器重排序**、**每文件多样性上限**及**图扩展**（调用者/被调用者等）
...
- **仪表盘** — React + Vite SPA（搜索、深度搜索、图探索、仓库管理、索引、Wiki、同步、设置）
...
仪表盘是一个**单页应用**，包含多个路由视图：**搜索**（混合自然语言查询）、**深度搜索**（LLM 多步推理，需配置 LLM）、**图探索**（实体邻域）、**仓库**（已索引仓库及统计）、**索引**（触发任务）、**文档**、**Wiki**、**同步**（定时计划）、**业务**及**设置**。图表和图布局按路由**按需加载**，减小初始包体积。
