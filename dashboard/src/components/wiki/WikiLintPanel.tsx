...
function severityBadgeClass(sev: WikiLintIssue["severity"]): string {
  if (sev === "error") return "bg-red-100 text-red-800 ring-red-200";
          ...
          <ul className="max-h-[min(60vh,520px)] space-y-2 overflow-y-auto pr-1">
            {report.issues.map((issue, i) => (
              <li
                ...
                className="rounded-lg border border-gray-100 bg-gray-50/80 px-3 py-2.5 text-sm shadow-sm"
              >
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded-md bg-white px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-gray-600 ring-1 ring-gray-200">
                    {formatCategory(issue.category)}
                  </span>
                </div>
                <p className="mt-1.5 text-gray-900">{issue.message}</p>
                ...
                {issue.suggestion && (
                  <p className="mt-1.5 text-xs leading-relaxed text-gray-600">{issue.suggestion}</p>
                )}
              </li>
            ))}
          </ul>
