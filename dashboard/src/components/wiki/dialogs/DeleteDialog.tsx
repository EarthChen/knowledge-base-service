...
export default function DeleteDialog({ domainTitle, onConfirm, onCancel }: DeleteDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onCancel}>
      ...
      >
        <h3 className="mb-2 text-lg font-semibold text-gray-900 dark:text-white">{t.wiki.domain_management.deleteTitle}</h3>
        ...
        <div className="mb-4 space-y-2">
          <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300">
            <input type="radio" checked={promoteChildren} onChange={() => setPromoteChildren(true)} className="accent-sky-600" />
            {t.wiki.domain_management.promoteChildren}
          </label>
          <label className="flex items-center gap-2 text-sm text-red-600 dark:text-red-400">
            <input type="radio" checked={!promoteChildren} onChange={() => setPromoteChildren(false)} className="accent-red-600" />
