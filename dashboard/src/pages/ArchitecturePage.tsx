...
function layerBadgeClasses(layer: string): string {
  ...
  if (k.includes("presentation")) return "bg-sky-100 text-sky-800 border-sky-200";
  if (k.includes("business") || k === "biz") return "bg-violet-100 text-violet-800 border-violet-200";
  if (k.includes("model")) return "bg-emerald-100 text-emerald-800 border-emerald-200";
  ...
  if (k.includes("rpc")) return "bg-blue-100 text-blue-800 border-blue-200";
  if (k.includes("messaging") || k.includes("msg")) return "bg-orange-100 text-orange-800 border-orange-200";
  if (k.includes("unknown")) return "bg-gray-100 text-gray-800 border-gray-200";
        ...
        <div className="min-w-0 flex-1 space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <input
              ...
              className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 sm:max-w-xs"
            />
            <select
              value={repositoryFilter}
              onChange={(e) => setRepositoryFilter(e.target.value)}
              className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 sm:w-56"
            >
            ...
            <table className="w-full min-w-[640px] text-left text-sm">
              <thead className="border-b border-gray-200 bg-gray-50">
                <tr>
                  ...
                  <th className="px-3 py-3 font-medium text-gray-500">{t.architecture.name}</th>
                  <th className="px-3 py-3 font-medium text-gray-500">{t.architecture.fqn}</th>
                    ...
                    return (
                      <Fragment key={row.uid}>
                        ...
                        >
                          ...
                          <td className="max-w-xs truncate px-3 py-2.5 font-mono text-xs text-gray-600">
                            {row.fqn ?? "—"}
                          </td>
                          <td className="max-w-[200px] truncate px-3 py-2.5 text-gray-600">
                            {row.repository ?? "—"}
                          </td>
                          <td className="max-w-[180px] px-3 py-2.5 text-xs text-gray-600">
                        ...
                        {isOpen && methodCount > 0 && (
                          <tr className="border-b border-gray-100 bg-gray-50/80">
                            <td colSpan={6} className="px-3 pb-4 pt-0">
                              <p className="mb-2 pl-7 text-xs font-medium text-gray-500">
                              ...
                              <ul className="space-y-2 pl-7">
                                {row.methods.map((m) => (
                                  ...
                                  >
                                    ...
                                    <div className="mt-1 font-mono text-[11px] leading-relaxed text-gray-600">
                                      <span className="text-gray-400">{t.architecture.signature}: </span>
                                      {m.signature}
                                    </div>
                                  </li>
                                ))}
