...
**状态**: ✅ Mostly Complete (核心全部完成 + Pipeline 已接入; 4 项 P2 优化迁移至剩余提案)  
...
## 11. 实施完成度追踪 (2026-05-08 最终更新)

### Layer 1: P0 Bug Fix — ✅ 全部完成
| 节 | 内容 | 状态 | 备注 |
|----|------|------|------|
| 3.1 | CONTEXT_GAP 正则统一 | ✅ | `wiki/context_gap.py` |
| 3.2 | cleanup 全路径覆盖 | ✅ | heal.py + aggregate.py |
| 3.3 | Markdown 围栏剥离 | ✅ | json_robust.py + topic_page_composer.py |
| 3.4 | tree_linker 截断修复 | ✅ | `_safe_truncate` + 中文标题 |
| 3.5 | 进度 callback 透传 | ✅ | service.py |
| 3.6 | 测试清单 | ✅ | 全部覆盖 |

### Layer 2: Graph Data Fix — ✅ 全部完成
| 节 | 内容 | 状态 | 备注 |
|----|------|------|------|
| 4.1 | CONTAINS 补全策略 | ✅ | `indexer/post_process.py` |
| 4.1 | 接入 indexer 流程 | ✅ | `incremental_indexer.py` 调用 `supplement_contains_relationships` |
| 4.2 | CCB Cypher 查询改造 | ✅ | `call_chain_cypher` 返回 caller/callee_functions |
| 4.2 | content_context_builder 适配 | ⚠️ 部分 | CCB 未直接使用 caller_functions/callee_functions 列 → 迁移至剩余提案 |
| 4.3 | 物化 Module-level CALLS | ⏭️ 跳过 | 设计中标记为可选，当前方案可工作 |

### Layer 3: Domain Classification Fix — ✅ 核心完成
| 节 | 内容 | 状态 | 备注 |
|----|------|------|------|
| 5.1 | 域分类优化 | ⚠️ | 反幻觉 prompt ✅, 图拓扑预分组 + 目录辅助 → 迁移至剩余提案 |
| 5.2 | 小域合并后处理 | ✅ | `wiki/domain_merger.py` + `dependency_graph.py` 已接入 |
| 5.3 | Overview 反幻觉 prompt | ✅ | `SYSTEM_DOMAIN_CLASSIFICATION` |
| 5.4 | 小域页面合并 | → | 迁移至剩余提案 |
| 5.5 | Overview 自底向上合成 | ✅ | `overview_synthesizer` 已接入 `tree_linker` |

### Layer 4: Agent-Driven Engine — ✅ 核心完成
| 节 | 内容 | 状态 | 备注 |
|----|------|------|------|
| 6.2 | WikiPageAgent.generate() | ✅ | `wiki/page_agent.py` |
| 6.3 | Agent System Prompt | ✅ | `wiki/agent_prompts.py` |
| 6.4 | 路由策略 + 配置开关 | ✅ | `AgentConfig` 已接入 `compose.py` |
| 6.5 | 拓扑排序 + SCC | ✅ | `topo_sort` 已接入 compose ordering |
| 6.6 | 源码引用验证 | ✅ | `citation_verifier` 已接入 `quality_gate_node` |
| 6.7 | Fallback + 配置 | ✅ | generate() 内置 skeleton fallback |

### 剩余项 (迁移至统一剩余提案)
| # | 内容 | 优先级 | 说明 |
|---|------|--------|------|
| R1 | CCB 适配 caller_functions/callee_functions | P2 | 增强调用链上下文展示 |
| R2 | 图拓扑预分组 + 目录辅助域分类 | P2 | 提升域分类准确度 |
| R3 | 小域页面合并 (1-2 模块 → 单页) | P2 | compose 阶段合并 |
| R4 | 部署验证 + 全量重新生成 | P1 | 需要生产环境执行 |
