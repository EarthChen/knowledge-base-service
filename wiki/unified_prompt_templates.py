...
- 在正文中用 `ClassName` 或 `ClassName.methodName()` 引用类名和方法名即可，无需附带文件路径或行号。
- **绝对禁止**编造文件路径或行号——即使参考数据中列出了 `source://` 链接，也不要在你的输出中复制或仿写它们。
- **必须嵌入代码片段**：每篇域文档**至少包含 3 个**带语言标记的代码块（如 ```java），展示关键方法签名、核心逻辑片段或配置代码。代码片段必须来自参考数据中的「关键代码片段」章节，每段不超过 15 行。
...
- **必须嵌入代码片段**：文档中**至少包含 3 个**带语言标记的代码块（如 ```java），从「关键代码片段」中选取，每段不超过 15 行
...
4. ## 关键入口与核心服务 — 逐一描述入口/核心业务模块的**职责、关键方法签名、典型调用场景**，附仓库名（不要自行附加 source:// 链接，系统会自动注入）。在说明核心入口逻辑时，从「关键代码片段」中选取 2～3 段最重要的代码嵌入正文（使用 ```java 等带语言标记的代码块，每段不超过 15 行），解释其业务含义
