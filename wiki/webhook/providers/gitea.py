"""Gitea push webhook payload parser."""
...
class GiteaWebhookParser:
    @staticmethod
    def parse_push(headers: dict[str, str], payload: dict[str, Any]) -> WebhookEvent | None:
        """Parse Gitea push webhook payload into WebhookEvent."""
        event_name = headers.get("x-gitea-event", "").lower()
        if event_name != "push":
            return None
        ...
        delivery_id = GiteaWebhookParser.extract_delivery_id(headers)
        ts = GiteaWebhookParser._push_timestamp(payload)
        changed = GiteaWebhookParser.extract_changed_files(commit_dicts)
    ...
    def extract_delivery_id(headers: dict[str, str]) -> str:
        ...
        val = headers.get("x-gitea-delivery")
